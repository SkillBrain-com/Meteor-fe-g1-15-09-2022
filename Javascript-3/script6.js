...
avgProfit = sumProfit / pretCumparare.length;
console.log(`Media aritmetica a tuturor masinilor este ${Math.floor(avgProfit)}`);

for (let i=0; i<pretCumparare.length; i++) {
    profit = pretVanzare[i] - pretCumparare[i];
    if(profit > avgProfit) {
        console.log(`${Marca[i]} ${Model[i]} are profitul peste medie: ${profit}`);
    }
}
